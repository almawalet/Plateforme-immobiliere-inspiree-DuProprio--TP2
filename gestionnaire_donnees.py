...
def sauvegarder_utilisateurs(utilisateurs):
    ...
    """
    tous_les_utilisateurs = charger_utilisateurs()

    tous_les_utilisateurs.update(utilisateurs) #Methode .update() trouver en ligne

    with open(FICHIER_UTILISATEURS, "w") as fichier:
        fichier.write(f'utilisateur, hash \n')

        for chaque_utilisateur in tous_les_utilisateurs:
            fichier.write(
                f"{chaque_utilisateur},{tous_les_utilisateurs[chaque_utilisateur]}\n"
            )
    ...
    with open(FICHIER_PROPRIETES, "r") as fichier:
        if fichier.readline().strip():
            for ligne in fichier:
                ligne = ligne.strip()
                if ligne:
                    prix, ville, type, chambres, salles_de_bains = ligne.split(",")
                    prix = int(prix)
                    ville = str(ville)
                    type = str(type)
                    chambres = int(chambres)
                    salles_de_bains = int(salles_de_bains)
                    propriete = {"prix": prix, "ville": ville, "type": type, "chambres": chambres, "salles_de_bains": salles_de_bains}
